...
function draw() {
  ...
  for (let rectX = -500; rectX <= 600; rectX += 50) {
    ...
    for (let b = 0; b < width; b += 3) {
      stroke(coswave[b] * 255);
      line(i, y1, b, y2);
    }
  }
  ...
  for (let rectX = -100 - 600; rectX <= 600; rectX += 50) {
    branches = [0];
  branch1 = new Branch();
    for (let branchesDrawn = 0; branchesDrawn < 50; branchesDrawn++) {
       Branch[branchesDrawn] = new Branch();
...
function Branch(){
  ...
  function randomShaped() {
    ...
    triangle(
      random(width),
      random(height),
      random(width, random(width), random(height), random(width))
    );
    noLoop();
